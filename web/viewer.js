...
(() => {
  ...
  // Brain-panel hover previews: hovering a backlink / surfaced / on-this-day
  // item shows a floating card with the note's title + first ~160 chars, so you
  // can glance a connection without leaving the note. Fetches /raw (cached),
  // debounced; the card is pointer-events:none so it never steals the hover.
  (function attachBrainPreviews() {
    ...
    function notePathFromHref(href) {
      ...
      m = href.match(/\/note\/([^?#]+)/);         // viewer / backlink links
      if (m) return decodeURIComponent(m[1]);
      return '';
    }
    function ensureCard() {
    ...
    function show(anchor, data) {
      ...
      c.appendChild(b);
      c.hidden = false;
      // Prefer left of the right-hand panel; fall back to staying on-screen.
